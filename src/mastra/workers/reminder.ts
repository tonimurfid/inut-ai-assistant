...
cron.schedule('0 20 * * 0', async () => {
  ...
  const todos = db.prepare(`
    ...
    FROM todos WHERE created_at >= datetime('now', '-7 days')
  `).get() as any;
